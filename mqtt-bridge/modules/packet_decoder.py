...
def decode_packet(eth_pkt,debug):
    ...
    if packet_type == 0x1F: # Frequency Message
        ...
        data = {
            ...
            freq = int.from_bytes(eth_pkt[0x22:0x24], "little") 
            div_wh = int.from_bytes(eth_pkt[0x2c:0x2e], "little")
            gen_pow = int.from_bytes(eth_pkt[0x3c:0x3e], "little",signed=True)
        ...
        v1 = int.from_bytes(eth_pkt[0x3C:0x3E], "little",signed=True)
        v2 = int.from_bytes(eth_pkt[0x44:0x46], "little",signed=True)
        v3 = int.from_bytes(eth_pkt[0x3A:0x3C], "little",signed=True) # Frequency
        v4 = int.from_bytes(eth_pkt[0x38:0x3A], "little",signed=True) # Voltage?
        v5 = int.from_bytes(eth_pkt[0x36:0x38], "little",signed=True)
        v6 = int.from_bytes(eth_pkt[0x30:0x32], "little",signed=True)
        v7 = int.from_bytes(eth_pkt[0x26:0x28], "little",signed=True)
        v8 = int.from_bytes(eth_pkt[0x24:0x26], "little",signed=True)
        v9 = int.from_bytes(eth_pkt[0x22:0x24], "little",signed=True)
        if debug:
            data = {
                'serial':serialno,
                '0x36/v1':v1,
